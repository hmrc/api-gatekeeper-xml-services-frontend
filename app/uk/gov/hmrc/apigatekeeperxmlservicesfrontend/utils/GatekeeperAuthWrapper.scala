...
import uk.gov.hmrc.apigatekeeperxmlservicesfrontend.models.{GatekeeperRole, LoggedInRequest, LoggedInUser}
import uk.gov.hmrc.apigatekeeperxmlservicesfrontend.models.GatekeeperRole.GatekeeperRole
    ...
    Action.async {
      implicit request: Request[AnyContent] =>
        ...
        authConnector.authorise(predicate, retrieval) flatMap {
          case Some(name) ~ authorisedEnrolments => { println(s"****GatekeeperAuthWrapper.requiresAtLeast*** ${name.name}")
